...
**Revision:** 8 · **Updated:** 2026-07-02 · **Status:** `codeserver-1.0.0-dev-0.0.4` **RELEASED** (`0e2f854`, tag on all 4 mirrors, GitHub+GitLab prereleases). Release gate: full §11.4.169 matrix **29/29 suites PASS, 0 FAIL** (61 checks PASS, 24 honest SKIP; evidence `qa-results/run_all/20260701T222753Z-2813294`). **Post-release coverage promotion (§11.4.52/§11.4.123):** wiring an authorized `HELIX_TEST_SSH_KEY` (gitignored `deploy/.env`; §11.4.10 never committed/logged) promoted the credential-gated auth SKIPs into **real authenticated-session PASSes**, and a non-destructive **isolated throwaway-gate** C3 variant (§11.4.85/§11.4.101 — own loopback port, throwaway keypair/secret, no live-gate rotation, no real credential) closed the LAST `credential_absent` SKIP — matrix **29/29 suites PASS, 0 FAIL, 72 checks PASS / 16 SKIP, ZERO `credential_absent`** (evidence `qa-results/run_all/20260702T000047Z-3718172`); the 16 remaining SKIPs are all honest (13 `topology_unsupported` = 11 legacy-suite retirements + 2 Chromium-contended pixel proofs, 2 `operator_attended` with passing autonomous CLI siblings, 1 `feature_disabled_by_config` = `load_auth` L3 login-flood rate-limit inconclusive, where the limiter is positively confirmed by `security_auth` #4's real 429). C3's §1.1 pair binds: it PASSes on the real gate (200/200/401/200) and FAILs on a gate with the cookie HMAC check disabled. Ships: login-redirect fix + login-form copy/paste clipboard buttons + Open VSX marketplace install/use/persist/config + popular extensions + VS Code Dark default theme (host-rendered pixel proof) + durable `systemd --user` edge. Three test-infra suites hardened against shared-host `ulimit -u` fork-pressure false-FAILs (§11.4.1/§11.4.3/§11.4.174) + independently reviewed to a clean GO (§11.4.142/§11.4.134). Stack LIVE at https://192.168.0.213:52443 (all `systemd --user`).
...
1. **C3 throwaway-gate variant — DONE** (isolated `helix-auth` on its own loopback
   port + throwaway keypair/secret; `stress_chaos_auth` C3 now runs autonomously +
   non-destructively, closing the last `credential_absent` SKIP; §1.1 pair binds).
   Fixture helpers `hc_spawn/restart/stop_throwaway_gate` in `tests/lib/stack_fixture.sh`.
